...
export class SaveInteractionMemoryTool {
  ...
  @Tool({
    ...
    description: 'Save a conversation event. Indexes in Qdrant only if role=system and isSummary=true.',
  ...
  async run(input: SaveInteractionMemoryInput): Promise<SaveInteractionMemoryOutput> {
    process.stderr.write(`[SaveInteractionMemoryTool] project=${input.projectName}\n`);
