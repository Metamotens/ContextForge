...
ALTER TABLE conversations ADD COLUMN IF NOT EXISTS model VARCHAR(120);
ALTER TABLE conversations ADD COLUMN IF NOT EXISTS title VARCHAR(255);

UPDATE conversations SET model = 'unknown' WHERE model IS NULL;

DO $$
BEGIN
  ALTER TABLE conversations ALTER COLUMN model SET DEFAULT 'unknown';
